...
```
Run MM-GBSA in parallel. Set `-np 12` to the number of CPU cores available.
```bash
...
| File | What to edit |
|---|---|
| `parameterise.py` | `protein_pdb`, `ligand_pdb` (input names), `net_charge` (ligand formal charge), `box_padding` |
| `equilibration.py` | `NVT_Time_ns`, `NPT_Time_ns`, `Temperature`, `Pressure` |
| `production.py` | `Jobname`, `REPLICA_ID`, `Stride_Time`, `Number_of_strides`, `Temperature`, `Pressure` |
| `cpptraj.in` | `trajin` filename (must match production output); strip mask if ion names differ |
| `rms.py` | `protein_indices` residue range, `ligand_indices` residue name, `total_time_ns`, output image names |
| `hbond_analysis.py` | Ligand residue name (`'LIG'`), output image name |
| `mmgbsa.in` | `startframe`, `endframe`, `interval` (must fit the trajectory length) |

## Known Gotchas

- Run `setup_paths.sh` once after cloning. It replaces the `__PROJECT_DIR__` placeholder in all scripts with your current folder path.
- `tleap.in`, `packmol.inp`, and `prepareforleap.in` are regenerated by `parameterise.py`. Edit settings in `parameterise.py`, not in these files.
- `ligand.pdb` must contain hydrogens, or antechamber will fail.
- Production writes `{Jobname}_rep{ID}_{n}.dcd` (e.g. `500ns_rep1_1.dcd`). Set `trajin` in `cpptraj.in` to this exact name.
- `rms.py` must load `nopbc.xtc` with `nopbc.prmtop`. The full `SYS_gaff2.prmtop` includes water, so the atom count will not match.
- The ligand residue number for `ante-MMPBSA.py` differs per system. Find it with `cpptraj` `resinfo` (see the MM-GBSA section).
- Residue numbers in the analysis output use Amber sequential numbering, which may differ from the original PDB numbering.
